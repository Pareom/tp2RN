...
try:
    from ift725.im2col_cython import col2im_cython, im2col_cython
    from ift725.im2col_cython import col2im_6d_cython
except ImportError:
    print('run the following from the ift725 directory and try again:')
    print('python setup.py build_ext --inplace')
    print('You may also need to restart your iPython kernel')

from ift725.im2col import *
...
def conv_forward_strides(x, w, b, conv_param):
    ...
    H += int(2 * pad)
    W += int(2 * pad)
    out_h = np.uint32((H - HH) / stride + 1)
    out_w = np.uint32((W - WW) / stride + 1)
